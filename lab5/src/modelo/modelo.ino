...
void setup()
{
    ...
    while (!Serial);
    Serial.println("Inferencia de actividad humana utilizando giroscopio\r\n");

    /* Connect used sensors */
    if(ei_connect_fusion_list(EI_CLASSIFIER_FUSION_AXES_STRING) == false) {
        ei_printf("ERR: Errors en lista de sensores\r\n");
        return;
    }
    ...
    for(int i = 0; i < fusion_ix; i++) {
        if (sensors[fusion_sensors[i]].status == NOT_INIT) {
            ...
            if (!sensors[fusion_sensors[i]].status) {
              ei_printf("%s Fallo en la inicializacion de sensor de eje.\r\n", sensors[fusion_sensors[i]].name);             
            }
            else {
              ei_printf("%s Inicializacion de sensor de eje exitosa.\r\n", sensors[fusion_sensors[i]].name);
            }
        }
    }
}
...
void loop()
{
    ei_printf("\nComenzando inferencia en 1 segundo:\r\n");
    ...
    ei_printf("    puntaje de anomalia: %.3f\r\n", result.anomaly);
#endif
}
